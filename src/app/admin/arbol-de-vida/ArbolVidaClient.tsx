...
export default function ArbolVidaClient({ rows }: { rows: SolicitudRow[] }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6" style={{ fontFamily: 'var(--font-dm-sans)' }}>
      <div className="section-heading">
        <h1>Árbol de Vida</h1>
        <p>Afiliados interesados (clic en &quot;Próximamente&quot;) para seguimiento comercial.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        <div className="stat-card">
          ...
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Interesados en el periodo</span>
        </div>
        <div className="stat-card">
          <span className="stat-label">Pendientes</span>
          <span className="stat-value" style={{ color: '#fbbf24' }}>{stats.pendiente}</span>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Aún no contactados</span>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          ...
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Contrataron el servicio</span>
        </div>
      </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: '48px 20px', textAlign: 'center', color: 'rgba(255,255,255,0.4)' }}>
                    Sin interesados para los filtros seleccionados.
                  </td>
                </tr>
              ) : (
                filtered.map((r) => (
